     ...
     del=async (_id)=>{
      ...
      handleOk=async ()=>{
       ...
       let result= await Adminsapi.add(userName,passWord)
        ...
        this.refreshList()
       let amend= await Adminsapi.add(userName,passWord)
       if(amend.code!==0){return notification.error({description:'管理员修改失败，请详细检查输入信息',message:'错误',duration:1.5})}
       notification.success({description:'管理员x成功，对话框即将关闭',message:'成功',duration:1.5})
       this.setState({visible:false})
       this.refreshList()
